...
import {
  ...
  Package, TrendingUp, Eye, EyeOff, Save
} from 'lucide-react';
...
const EMPTY: Omit<Product, 'id' | 'created_at' | 'updated_at'> = {
  ...
  stock: 0, image_url: '', category: 'watch', is_active: true,
};
...
export default function AdminProductsPage() {
  ...
  async function load(q = '') {
    setLoading(true);
    const url = q ? `/api/products?all=true&search=${encodeURIComponent(q)}` : '/api/products?all=true';
  ...
  function openEdit(p: Product) {
    ...
    setForm({
      ...
      category: p.category || 'watch', is_active: p.is_active,
    });
    setModalOpen(true);
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-obsidian-400 font-body text-sm mt-0.5">{products.length} produits au total</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-obsidian-700">
                {['Produit', 'Prix achat', 'Prix vente', 'Marge', 'Stock', 'Statut', 'Actions'].map(h => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-obsidian-800 rounded-2xl border border-obsidian-700 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-obsidian-700">
              <h2 className="font-display text-2xl text-white">{editing ? 'Modifier le produit' : 'Nouveau produit'}</h2>
            ...
            <div className="p-6 space-y-5">
              {/* Image Upload */}
              <div>
                <label className="block text-sm font-body font-medium text-obsidian-300 mb-2">Image du produit</label>
                <div className="flex items-center gap-4">
                  <div className="w-24 h-24 rounded-xl bg-obsidian-700 overflow-hidden flex items-center justify-center border border-obsidian-600 flex-shrink-0">
                    {form.image_url
                      ? <Image src={form.image_url} alt="preview" width={96} height={96} className="object-cover w-full h-full" />
                      : <Clock className="w-8 h-8 text-obsidian-500" />}
                  </div>
                  <div className="flex-1 space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-obsidian-700 border border-obsidian-600 text-white placeholder-obsidian-500 rounded-xl px-3 py-2 text-sm font-body focus:outline-none focus:ring-2 focus:ring-gold-500"
                    />
                  </div>
                </div>
              </div>
              ...
              <Field label="Nom du produit *">
                <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                  placeholder="Rolex Submariner..." className={inputCls} />
              </Field>

              {/* Description */}
              <Field label="Description">
                <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
                  rows={2} placeholder="Description courte..." className={`${inputCls} resize-none`} />
              </Field>
              ...
              {form.selling_price > 0 && (
                <div className="flex items-center gap-2 p-3 bg-obsidian-700/50 rounded-xl border border-obsidian-600">
                  ...
                  <p className="text-sm font-body text-obsidian-300">
                    ...
                    {' '}— Profit unitaire: <span className="text-emerald-400 font-medium">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Catégorie">
                  <input value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))}
                    placeholder="watch" className={inputCls} />
                </Field>
              </div>
...
const inputCls = "w-full bg-obsidian-700 border border-obsidian-600 text-white placeholder-obsidian-500 rounded-xl px-4 py-2.5 font-body text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent";

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-sm font-body font-medium text-obsidian-300 mb-1.5">{label}</label>
      {children}
    </div>
  );
}
